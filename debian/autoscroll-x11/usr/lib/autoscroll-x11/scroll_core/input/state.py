...
class ScrollMode(Enum):
    ...
    ARMED = auto()   # Middle button pressed; awaiting movement threshold.
    ACTIVE = auto()  # Movement threshold exceeded; autoscroll running.
...
class ScrollState:
    ...
    # Press position in root-window coordinates (set on button press).
    press_x: int = 0
    press_y: int = 0

    # Anchor position in root-window coordinates (set on activation).
    anchor_x: int = 0
    anchor_y: int = 0
